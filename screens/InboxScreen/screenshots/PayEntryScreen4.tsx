...
export default function PayEntryScreen4() {
  const navigation = useNavigation<NavigationProp<any>>();
  const [friendChannelUrl, setFriendChannelUrl] = useState('');

  //Navigate to Chat and Call Screen
  useEffect(() => {
    getChannelUrlByCustomType('paydemo').then((channelUrl) => setFriendChannelUrl(channelUrl));
  }, []);
  ...
  useEffect(() => {
    async function loadCallee() {
      const hailey = await getCallee(botUserIds.hailey);
      setCalleeUser(hailey);
    }
    ...
    <View style={{ flex: 1 }}>
      ...
      <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: 'white' }}>
        <View style={{ position: 'relative' }}>
          <AssetBlockImage source={require('../../../assets/PaymentSuccess1.png')} />
          <Hotspot style={{ position: 'absolute', width: '15%', height: '7%', left: '37%', right: '30%', top: '52%' }} onPress={() => {
            navigation.navigate('Chat', { channelUrl: friendChannelUrl });
          }} />
          <Hotspot
            style={{ position: 'absolute', width: '15%', height: '7%', left: '53%', right: '10%', top: '52%' }}
            onPress={() => {
              if (calleeUser) {
